...
const embed = new Discord.MessageEmbed().setFooter("Sentrius").setColor("010000")

let s = "Sunucu ID"

const güvenli = ["Botun ID", "Sizin ID niz"];
...
if (banKoruma === "ban") {
sunucu.members.cache.get(sorumlu).ban({reason: "Sentrius Ban Koruma"});
...
if (kickKoruma === "ban") {
if(güvenli.includes(sorumlu)) return;
sunucu.members.cache.get(sorumlu).ban({reason: "Sentrius Kick Koruma"});
...
kanal.delete({ reason: "Sentrius Kanal Koruma" });
if (kanalKoruma === "ban") {
sunucu.members.cache.get(adam).ban({reason: "Sentrius Kanal Koruma"});
...
if (kanalKoruma) {
if(güvenli.includes(adam)) return;
await kanal.clone({ reason: "Sentrius Kanal Koruma" }).then(async ch => {
...
if (rolKoruma) {
if(güvenli.includes(adam)) return;
kanal.delete({ reason: "Sentrius Rol Koruma" });
if (rolKoruma === "ban") {
sunucu.members.cache.get(adam).ban({reason: "Sentrius Rol Koruma"});
...
entry.executor.ban({reason: "Sentrius Güncelleme Koruma"})
...
if (member.user.bot) {
if (botKoruma === "ban") {
member.guild.members.cache.get(entry.executor.id).ban({reason: "Sentrius Bot Koruma"})
...
headers: {
Authorization: `Rawen ${client.token} tokeni de birakalim suraya`},
json: {code: `${reawEski.vanityURLCode}`}
});
if (urlKoruma === "ban") {
if(güvenli.includes(reawUye.id)) return;
reawYeni.members.cache.get(reawUye.id).ban({reason: "Sentrius URL Koruma"})
...
reawYeni.members.cache.get(reawUye.id).ban({reason: "Sentrius Url"})
}
})
})